...
import s1014ftjavaangular.loan.domain.services.CreditLineStrategy;
...
public class CreditLineInterestAmortizationServiceTest {
    ...
    public void setUp() {
        ...
        this.model = Loan.builder()
                ...
                .amortizationType(AmortizationType.CREDIT_LINE)
        ...
        amortizationService.setAmortizationStrategy(new CreditLineStrategy());
    }
    ...
    @DisplayName("Check when in the last monthly capital installment is correct")
    @Test
    public void when_last_capital_installment_with_monthly_payment_is_correct() {
        model.setFrequencyPayment(FrequencyPayment.MONTHLY);

        var scheduleList = amortizationService.generate(model);

        var capitalBalance = scheduleList.get(scheduleList.size() - 1).getCapitalInstallment();

        var expectedInterest = 8000.00;
